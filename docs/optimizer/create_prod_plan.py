...
def _handle_changeover(workplaces, step, choosen_spaces, wp_id, start, space):
    ...
    #next_color = workplaces.get_nearest_next_color(wp_id, start)
    next_color = None

    changeover_prev = 0 if (prev_color is None or step.color == prev_color) else changeover_time_secs
    changeover_next = 0 if (next_color is None or step.color == next_color) else changeover_time_secs

    if (prev_color is None or step.color == prev_color) and (next_color is None or step.color == next_color):
        choosen_spaces[wp_id][-1]["changeover"] = changeover_prev
        choosen_spaces[wp_id][-1]["changeover_next"] = changeover_next
        choosen_spaces[wp_id][-1]["color"] = step.color
        return True

    start_with_changeover = max(space["start"], start - timedelta(seconds=changeover_prev))
    start_with_changeover += timedelta(seconds=changeover_prev)
    end_with_changeover = start_with_changeover + timedelta(seconds=choosen_spaces[wp_id][-1]["dur"])


    if end_with_changeover + timedelta(seconds=changeover_next) <= space["end"]:
        choosen_spaces[wp_id][-1].update({
            "start": start_with_changeover,
            "end": end_with_changeover,
            "changeover": changeover_time_secs,
            "changeover_next": changeover_next,
            "color": step.color
        })
        return True
    else:
